...
LAB_NAME=${PWD##*/}
LAB_REGION=$(jq -Mrc '.region.cluster' "../${SET_DEFAULT_CONFIG_FILE_CLUSTER}")
LAB_CLUSTER="unknown"
if test -f "../${SET_DEFAULT_CLUSTER_IDENT_FILE}"; then
    LAB_CLUSTER=$(cat "../${SET_DEFAULT_CLUSTER_IDENT_FILE}")
fi

# -- define workload related help output ------------------------------------------------------------------------------
...
do
    ...
    case "$1" in
        ...
        deploy)
            preflight "${LAB_REGION}"
            do_start_deploy "${LAB_REGION}" "${LAB_CLUSTER}" "${LAB_NAME}" $OPT_DEBUG $OPT_QUIET $OPT_NOCHECK
            do_show_help_for_workload
            ;;
        destroy)
            preflight "${LAB_REGION}"
            do_start_destroy "${LAB_REGION}" "${LAB_CLUSTER}" "${LAB_NAME}" $OPT_DEBUG $OPT_QUIET $OPT_NOCHECK
            ;;
    esac
    shift
done
