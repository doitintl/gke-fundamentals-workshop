...
Now we can set the current k8s context to our lab exercise namespace `doit-lab-01-b` to make sure that every command set is run against this lab resources.

```bash
kubectl config set-context --current --namespace=doit-lab-01-b
```

## Application Clean-Up

```bash
kubectl delete -f elasticsearch-simple.yaml ;
```
